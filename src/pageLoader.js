...
const makeDirName = (url) => makeFileName(url).replace('.html', '_files');
...
const downloadResource = (resource, dirPath) => {
  ...
  return axios
    ...
    .then((response) => fs.writeFile(fileFullPath, response.data));
};
    ...
    .then((response) => {
      ...
      return fs
        ...
        .then(() => {
          ...
          return fs.writeFile(filePath, updatedHtml).catch(() => {
